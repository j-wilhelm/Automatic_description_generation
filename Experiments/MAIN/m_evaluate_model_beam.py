mark# These need to be at the top to allow for running on cluster
import os
import random
import sys
...
for file in config_files:
    ...
    for beam_size in [5, 7, 9]:
        print(f"Predicting using beam search with beam_size={beam_size}")
        start = time.time()
        results_dict = captmodel.evaluate_model(BLEU=True, ROUGE=True, img_list=captmodel.test_imgs,
                                                beam=beam_size, batch_size=batch_size)

        end_greedy = time.time()
        print(f"Predicting val set with greedy took {end_greedy - start} time.")
        # Save results_dict
        with open(os.path.join(cwd, "models", "Output", captmodel.webshop_name, "results",
                               f"results_dict{file[:-5]}_beam{beam_size}.json"), 'w') as f:
            json.dump(results_dict, f)
        K.clear_session()
